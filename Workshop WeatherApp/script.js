...
const renderHomePage = (data) => {
  ...
  mainEl.innerHTML = "";
  mainEl.innerHTML = `<p class = "first-card" >Data: ${date.toLocaleDateString("fr")}<br> Temp: ${
    data.current.temp
  }<br> Feels like: ${data.current.feels_like}<br> Wind speed: ${
    data.current.wind_speed
  }<br> Humidity: ${data.current.humidity}<br>
  Weather icon: <img src = "http://openweathermap.org/img/wn/${
    data.current.weather[0].icon
  }@2x.png"<br><br>
  Weather discription: ${data.current.weather[0].description}</p>`;
...
const renderAboutPage = (mainEl) => {
  main1.innerHTML = "";
  mainEl.innerHTML = "";
  mainEl.innerHTML = `<h1>Martin Todorov G4</h1>`;
...
function createCards(day) {
  return `<div class="daily-card">
        <p >Data: ${new Date(day.dt * 1000).toLocaleDateString("fr")}<br>
        Max/Min Temperature: ${day.temp.min}/${day.temp.max}<br>
        Weather icon: <img src = "http://openweathermap.org/img/wn/${
          day.weather[0].icon
        }@2x.png"<br><br>
        Humidity: ${day.humidity}<br>
        Weather discription: ${day.weather[0].description}
        </p>
     </div>`;
}
...
function hourlyPage(weatherData) {
  mainEl.innerHTML = "";
  main1.innerHTML = "";
  for (let hourlyData of weatherData.hourly) {
    main1.innerHTML += `<p class="paraHour">Time: ${new Date(
      hourlyData.dt * 1000
    ).toLocaleTimeString("fr")}<br> Temp: ${hourlyData.temp}<br> Feels like: ${
      hourlyData.feels_like
    }<br> Wind speed: ${hourlyData.wind_speed}<br> Humidity: ${
      hourlyData.humidity
    }<br>
    Weather icon: <img src = "http://openweathermap.org/img/wn/${
      hourlyData.weather[0].icon
    }@2x.png"<br><br>
    Weather discription: ${hourlyData.weather[0].description}</p>`;
  }
}
